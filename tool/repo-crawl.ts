	...
	async execute({ repo, depth = 2 }, ctx) {
		...
		try {
			...
			const [repoInfo, contents] = await Promise.all([
				fetchGH(`/repos/${owner}/${repoName}`, signal) as Promise<{
					description: string;
					stargazers_count: number;
					forks_count: number;
					updated_at: string;
				}>,
				fetchGH(`/repos/${owner}/${repoName}/contents`, signal) as Promise<
				...
				for (const dir of dirs) {
					...
					if (importantDirs.includes(dirName)) {
						try {
							...
							const subItems = subContents.map((i) =>
		...
		async function getTree(
			...
			try {
				const contents = (await fetchGH(
					...
					signal,
				)) as { type: string; name: string; path: string }[];
				let result = "";
	...
	async execute({ repo, query, maxResults = 10 }, ctx) {
		...
		try {
			...
			const results = (await fetchGH(
				...
				ctx?.abort,
			)) as { items: { path: string }[]; total_count: number };
